...
public class AnaliseFinanceiraService {
    ...
    public ResponsePagamentos findOrdensPagtoCondition(Integer idOficina, Boolean nfRealizada, Boolean pagtoRealizado) {
        List<OrdemDeServico> ordensPagtoPendente = ORDEM_SERVICO_SERVICE.findOrdemByNfRealizadaAndPagtRealizado(nfRealizada, pagtoRealizado, idOficina);

        if (pagtoRealizado) {
            var kpi = ORDEM_SERVICO_SERVICE.findKpiPagamentoRealizado(idOficina);
            return AnaliseFinanceiraMapper.toResponsePagamentoRealizado(
                    kpi.totalPagamentosRealizados(),
                    ordensPagtoPendente
            );
        } else {
            var kpi = ORDEM_SERVICO_SERVICE.findKpiPagamentoPendente(idOficina);
    ...
    public ResponseNotaFiscals findOrdensNotaFiscalCondition(Integer idOficina, Boolean nfRealizada, Boolean pagtoRealizado) {
        ViewNotaFiscal kpiNotaFiscal = ORDEM_SERVICO_SERVICE.findKpiNotaFiscal(idOficina);
        List<OrdemDeServico> ordensNotaFiscalpendente = ORDEM_SERVICO_SERVICE.findOrdemByNfRealizadaAndPagtRealizado(nfRealizada, pagtoRealizado, idOficina);
